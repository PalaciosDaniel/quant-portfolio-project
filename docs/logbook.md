...
ACLARACION: Vamos a explicar toda la movida de 6.3 Mathematical Optimization (Mean-Variance / Maximum Sharpe) por que es un lio. O seatu tienes por un lado el retorno esperado a 21 dias de cada una de las acciones en cada una de las fechas dentro del periodo de train/validation y nosotros vamos a coger las predicciones de ranking (que realmente se dan como retornos esperados solo que nos debemos de fijar no es la magnitud sino en la ordenación es justo lo que nos descargamos como .parquet al principio de este punto) por fold, es decir, si el fold 1 se entreno con 2010-2022 y valido con 2022-2024 nosotros cogemos las predicciones hechas entre 2022 y 2024 (para minimizar el riesgo de data lackage). la movida es que como tenemos 21 folds va a ver empresas en una fecha concreta que formen parte de dos conjuntos de validacion (pertenecientes a dos folds distintos) diferentes por lo que primero debemos promediar eso para tener una empresa en una fecha especifica asociada a su retorno promedio a 21 dias real (no tener esas duplicicdades). 

Una vez tenemos eso ordenamos todas las empresas en todas las fechas hacemos el ranking en cada uno de los dias (ordenamos las empresas de cada dia dentro de un top  500 y les asignamos un percentil. Ahora al contruir el mega ranking tendremos en cada posicion del top 500 global un empate dentro del top 1 (la mejor empresa de cada uno de los dias), dentro del top 2 (la segunda mejor de cada dia está ahi), etc. Luego ya separamos por deciles (obviamente cada decil del mega ranking conendrá ese decil específico de cada uno de los días). 
